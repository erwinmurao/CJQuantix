...
const Inventory = () => {
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      ...
      <div>
        <h2 className="font-bold text-gray-900 text-lg tracking-tight mb-3">Stock Overview Logs</h2>
        ...
        <div className="w-full overflow-x-auto rounded-xl border border-gray-200/80 shadow-xs bg-white">
          ...
          <div className="min-w-[950px] divide-y divide-gray-100">
            ...
            {/* Table Header Row */}
            <div className={`${rowGrid} bg-gray-50/70 font-bold uppercase tracking-wider text-gray-500 text-xs`}>
            ...
            <div className="divide-y divide-gray-100 text-gray-700 bg-white">
              {items.map((item, index) => (
                <div key={index} className={`${rowGrid} hover:bg-gray-50/40`}>
                  ...
                  <div className="font-semibold text-emerald-600">+{item.stockIn}</div>
                  <div className="font-semibold text-rose-600">-{item.stockOut}</div>
                  <div className="font-bold text-gray-900">{item.available}</div>
                  
                  {/* Calendar Timestamps */}
                  <div className="text-gray-500 text-xs">{item.dateAdded}</div>
                  <div className="text-gray-500 text-xs">{item.dateOut}</div>
